...
def is_salt(item):
    if 'Salt' not in item and 'Synthesis' not in item:
        return False
    return True

def is_like(str, keys, all):
    if all:
        for key in keys:
            if not str.contains(key):
                return False
        return True
    for key in keys:
        if str.contains(key):
...
def salt_modifier(level, index):
    if dictionary['account']['tasks']['meritsOwned']['world3'][6] == 5 or dictionary['account']['tasks']['meritsOwned']['world3'][6] > index:
        return refinery['ranks'][level]['power']
    return refinery['ranks'][level]['cost']
...
#Dictionary for refinery stats
json_file = open("data/refinery_data.json", "rt")
json_text = json_file.read()
refinery = json.loads(json_text)
...
# Total resources generated by 3D printer
printerTotals = []